  ...
  return (
    <div className="flex flex-col gap-8 text-sm lg:text-base">
      ...
      <div className="text-muted space-y-3 text-sm lg:text-base">
        ...
        <p>
          The DOM (Document Object Model) is the live tree of objects the
          browser builds from your HTML and holds in memory. When a page loads,
          the browser reads your markup once and turns it into a structure it
          can change on the fly. Every element becomes a{" "}
          <em className="text-body">node</em>, an object in that tree, and the
          nesting in your HTML becomes parent-and-child links between nodes: a{" "}
          <code className="text-code">&lt;p&gt;</code> inside a{" "}
          <code className="text-code">&lt;div&gt;</code> is a child of that div
          node.
        </p>
        <p>
          Here is what trips people up: that live tree and your HTML file are
          not the same thing. The file is read once to build the tree and then
          set aside. Everything after that happens in the DOM. A row you add in
          JavaScript shows up on screen instantly, while the HTML on disk still
          shows the empty list you first shipped. Right-click a busy page,
          choose View Source, and hunt for something you can plainly see on
          screen. If JavaScript put it there, it is not in the source at all.
        </p>
        <p>
          ...
          changes on the page. The dish is what people actually eat, and the
          cook (your JavaScript) keeps adjusting it after it leaves the kitchen,
      ...
      <div className="text-muted space-y-3 text-sm lg:text-base">
        ...
        <p>
          The DOM is the bridge between your code and what a user sees on
          screen. Every UI framework, from React to Svelte, ultimately reads and
          writes the DOM; once you can picture the tree, you have a model for
          why an interface behaves the way it does and where to reach in to
          change it.
        </p>
        <p>
      ...
      <div>
        ...
        <p className="text-muted mb-4">
          ...
          <code className="text-code">&lt;div&gt;</code>, what happens to the{" "}
          <code className="text-code">&lt;p&gt;</code> and{" "}
          <code className="text-code">&lt;span&gt;</code> nested inside it?
          Click a node to inspect it, add or remove nodes, and watch the panels
          below compare the HTML you wrote against the live DOM.
        </p>
        <DomDemo />
        <p className="text-body mt-6 mb-2">Try breaking it:</p>
        <ul className="text-muted list-inside list-disc space-y-1.5">
          <li>
            Delete the <code className="text-code">&lt;div&gt;</code>. The live
            DOM drops it and everything nested inside, so why does the Source
            panel turn red over those lines instead of simply removing them?
          </li>
          <li>
            Add a <code className="text-code">&lt;button&gt;</code> under{" "}
            <code className="text-code">&lt;body&gt;</code>. It shows up green
            in the live DOM and never appears in the Source, because the Source
            is frozen at what you first wrote.
          </li>
          <li>
            Select a node, add a child to it, then select its parent and watch{" "}
            <code className="text-code">children</code> climb by one in the
            inspector.
          </li>
        </ul>
      </div>
      ...
      <div className="text-muted space-y-3 text-sm lg:text-base">
        ...
        <p>
          ...
          or use the <span className="font-mono text-xs">×</span> to remove one.
          The two panels underneath hold the frozen source next to the live DOM,
          the recipe held up against the actual plate, so you can see exactly
